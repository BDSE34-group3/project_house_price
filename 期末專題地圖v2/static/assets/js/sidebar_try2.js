...
//型態
    ...
    openTypePopupBtn.addEventListener('click', function(event) {
        event.preventDefault();
    ...
    for (let o of data.extracted_data) {
        if (o.latitude && o.longitude) {
            let marker = L.marker([o.latitude, o.longitude])
                .bindPopup(`<img src="${o['房屋圖片']}" alt="房屋圖片" width="300％; height:300%;">
                <br><strong>ID：</strong> ${o['index']}<br>
                <br><strong>屋齡：</strong> ${o['屋齡']}<br>
                <br><strong>總坪數：</strong> ${o['權狀坪數']}<br>
                <br><strong>實際價格：</strong> ${o['售價總價']}<br>
                <br><strong>預測價格：</strong> ${o['模型_實際價格']}<br>`
