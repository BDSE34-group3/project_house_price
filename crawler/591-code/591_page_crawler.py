...
def set_logger():
    ...
    # 儲存在 log 當中的事件處理器
    file_handler = logging.FileHandler('log/591_page_crawler.log', mode='a', encoding='utf-8') # a: append, w: write
        ...
        try:
            if soup.select_one('div.house-pic-title'):
                ...
                for image in soup.select('div.pic-box-item'):
                    try:
                        temp_house_image_url.append(image.select_one('img.pic-box-img')['data-original'])
                    except:
                        continue
                single_entry[soup.select_one('div.house-pic-title').text] = temp_house_image_url
            else:
...
def get_total_house_info(house_partial_url_list: list, target: str):
    ...
    #(改這邊可以大家一起爬)#########################################################
    for house_partial_url in house_partial_url_list[0:5]:
        sleep(6)
        ...
        if count_to_save == save_at_threshold: #每達到設定值，就存一次，避免失敗
            with open(f'result-data/{target}_error_591_page_crawler_info.json', 'w', encoding='utf-8') as f:
                f.write(json.dumps(error_house_list, ensure_ascii=False, indent=4)) #最後結束時再存一次
            
            with open(f'result-data/{target}_591_page_crawler_info.json', 'w', encoding='utf-8') as f:
                f.write(json.dumps(total_house_info, ensure_ascii=False, indent=4))
            count_to_save = 0
            logger.info(f'已經存了{len(total_house_info)}筆資料')
    
    with open(f'result-data/{target}_error_591_page_crawler_list.json', 'w', encoding='utf-8') as f:
        f.write(json.dumps(error_house_list, ensure_ascii=False, indent=4)) #最後結束時再存一次
    
    with open(f'result-data/{target}_591_page_crawler_list.json', 'w', encoding='utf-8') as f:
        f.write(json.dumps(total_house_info, ensure_ascii=False, indent=4))
