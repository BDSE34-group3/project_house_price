...
def set_logger():
    ...
    # 儲存在 log 當中的事件處理器
    file_handler = logging.FileHandler('log/591_page_crawler_for_coordinate.log', mode='a', encoding='utf-8') # a: append, w: write
...
def get_single_page_coordinate(partial_url):
    try:
        ...
        WebDriverWait(driver, 10).until(
            ...
            (By.CSS_SELECTOR, "div.detail-house-title"))
            )
        sleep(1)
    except:
...
#(改這邊可以分開爬)###################################################################
    for house_partial_url in house_partial_url_list[0:5]:
        ...
        if count_to_save == save_at_threshold:
            with open(f'result-data/{file_name}_591_page_crawler_for_coordinate.json', 'w', encoding='utf-8') as f:
                f.write(json.dumps(total_house_coordinate, ensure_ascii=False, indent=4))

            with open(f'result-data/{file_name}_error_591_page_crawler_for_coordinate.json', 'w', encoding='utf-8') as f:
...
if __name__ == '__main__':
    ...
    total_house_coordinate = get_total_coordinate(house_partial_url_list, driver, f'{target}')
    driver.quit()
