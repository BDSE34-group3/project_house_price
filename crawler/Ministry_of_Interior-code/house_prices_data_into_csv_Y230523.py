...
def first_process_for_sql(df):
    ...
    #把二元變數改為0,1
    df['elevator_available'] = df['elevator_available'].map({'有':0, '無':1})
    df['management_org_available'] = df['management_org_available'].map({'有':0, '無':1})
...
if __name__ == '__main__':
    ...
    try:
        df.to_csv('house_price_csv_data/新北市_10101_11303_房價before.csv')
    except:
        logger.error(f'新北市_10101_11312_房價before_csv存取異常')
    df = first_process_for_sql(df)
    #connect_sql_to_insert_df(df)
    try:
        df.to_csv('house_price_csv_data/新北市_10101_11303_房價after.csv')
    except:
        logger.error(f'新北市_10101_11312_房價after_csv存取異常')
